...
public class Controller implements Initializable {
	...
	private void handleClicks(ActionEvent event) 
	{
		if (event.getSource() == btnResidentes)
		{
			...
			pnlStatus.setBackground(new Background(new BackgroundFill(Color.rgb( 78, 145, 214), CornerRadii.EMPTY, Insets.EMPTY)));
			pnResidentes.toFront();
			//animate the status panel
			new FadeIn(pnlStatus).play();
		}
		else
		if (event.getSource() == btnVisitantes)
		{
		...
		if (event.getSource() == btanParqueaderos)
		{
			lblStatusMin.setText("/home/Parqueaderos");
			lblStatus.setText("Parqueaderos");
			pnlStatus.setBackground(new Background(new BackgroundFill(Color.rgb( 25, 145, 56), CornerRadii.EMPTY, Insets.EMPTY)));
		...
		if (event.getSource() == btnConfiguracion)
		{
			...
			pnlStatus.setBackground(new Background(new BackgroundFill(Color.rgb( 150, 156, 22), CornerRadii.EMPTY, Insets.EMPTY)));
		...
		if (event.getSource() == btnCerrarSesion)
		{
			...
			pnlStatus.setBackground(new Background(new BackgroundFill(Color.rgb( 156, 22, 57), CornerRadii.EMPTY, Insets.EMPTY)));
